...
namespace Fast
{
    ...
    public static class FastMap
    {
                    ...
                    foreach (var temp in item.ExportedTypes)
                    {
                        Task.Factory.StartNew(() =>
                        {
                            ...
                            if (typeInfo.Namespace.Contains(nameSpace))
                            {
                                ...
                                BaseCache.Set<List<PropertyModel>>(key, cacheList, RedisDb.Properties);
                            }
                        });
                    }
                }
        ...
        public static void InstanceTable(Assembly[] list, string nameSpace, string dll, string dbKey = null)
        {
            ...
            {
                var listInfo = typeof(Fast.DataModel.Oracle.Data_MapFile).GetProperties(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly).ToList();
                var listAttribute = typeof(Fast.DataModel.Oracle.Data_MapFile).GetTypeInfo().GetCustomAttributes().ToList();                
                BaseTable.Check(query, "Data_MapFile", "Fast.DataModel.Oracle", listInfo, listAttribute);
            }

            if (query.Config.DbType == DataDbType.MySql)
            {
                var listInfo = typeof(Fast.DataModel.MySql.Data_MapFile).GetProperties(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly).ToList();
                var listAttribute = typeof(Fast.DataModel.MySql.Data_MapFile).GetTypeInfo().GetCustomAttributes().ToList();
                BaseTable.Check(query, "Data_MapFile", "Fast.DataModel.MySql", listInfo, listAttribute);
            }

            if (query.Config.DbType == DataDbType.SqlServer)
            {
                var listInfo = typeof(Fast.DataModel.SqlServer.Data_MapFile).GetProperties(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly).ToList();
                var listAttribute = typeof(Fast.DataModel.SqlServer.Data_MapFile).GetTypeInfo().GetCustomAttributes().ToList();
                BaseTable.Check(query, "Data_MapFile", "Fast.DataModel.SqlServer", listInfo, listAttribute);
            }
        }
        #endregion
        ...
        public static void InstanceMap(string dbKey = null)
        {
            ...
            foreach (var item in list.Path)
            {
                ...
                if (!BaseCache.Exists(key, RedisDb.Xml))
                {
                    ...
                    if (SaveXml(key, info, config, db))
                        BaseCache.Set<MapXmlModel>(key, temp,RedisDb.Xml);
                }
                else if ((BaseCache.Get<MapXmlModel>(key,RedisDb.Xml).LastWrite - info.LastWriteTime).Minutes != 0)
                {
                    foreach (var temp in BaseCache.Get<MapXmlModel>(key, RedisDb.Xml).FileKey)
                        BaseCache.Remove(temp, RedisDb.Xml);
                    ...
                    if (SaveXml(key, info, config, db))
                        BaseCache.Set<MapXmlModel>(key, model,RedisDb.Xml);
                }
            }
        ...
        public static List<T> ExecuteMap<T>(string name, DbParameter[] param, DataContext db = null, string key = null) where T : class, new()
        {
            ...
            if (BaseCache.Exists(name.ToLower(), RedisDb.Xml))
            {
        ...
        private static List<string> ReadXml(string path,ConfigModel config)
        {
            ...
            for (var i = 0; i < key.Count; i++)
                BaseCache.Set(key[i].ToLower(), sql[i], RedisDb.Xml);
        ...
        private static string GetMapSql(string name, ref DbParameter[] param, DataContext db, string key)
        {
            ...
            for (var i = 0; i <= BaseCache.Get(name.ToLower(), RedisDb.Xml).ToInt(0); i++)
            {
                #region 文本
                var txtKey = string.Format("{0}.{1}", name.ToLower(), i);
                if (BaseCache.Exists(txtKey, RedisDb.Xml))
                    sql.Append(BaseCache.Get(txtKey, RedisDb.Xml));
                #endregion

                #region 动态
                var dynKey = string.Format("{0}.format.{1}", name.ToLower(), i);
                if (BaseCache.Exists(dynKey, RedisDb.Xml))
                {
                    if (param != null)
                    {
                        ...
                        foreach (var temp in param)
                        {
                            ...
                            if (BaseCache.Exists(paramKey, RedisDb.Xml))
                            {
                                var flagParam= string.Format("{0}{1}", flag, temp.ParameterName.ToLower());
                                var tempKey = string.Format("#{0}#", temp.ParameterName.ToLower());
                                var paramSql = BaseCache.Get(paramKey, RedisDb.Xml).ToLower();
                                var condition = BaseCache.Get(conditionKey, RedisDb.Xml).ToStr().ToLower();
                                var conditionValue = BaseCache.Get(conditionValueKey, RedisDb.Xml).ToStr().ToLower();
                                switch (condition)
                                {
                                    case "isEqual":
                                        {
                                            if (conditionValue == temp.Value.ToStr())
                                            {
                                                ...
                                                {
                                                    tempParam.Remove(temp);
                                                    tempSql.Append(BaseCache.Get(paramKey, RedisDb.Xml));
                                                }
                                                else
                                                    tempSql.Append(BaseCache.Get(paramKey, RedisDb.Xml));
                                            }
                                    ...
                                    case "choose":
                                        {
                                            var isSuccess = false;
                                            for (int j = 0; j < BaseCache.Get(paramKey, RedisDb.Xml).ToStr().ToInt(0); j++)
                                            {
                                                conditionKey = string.Format("{0}.choose.{1}", paramKey, j);
                                                condition = BaseCache.Get(conditionKey, RedisDb.Xml).ToStr().ToLower();

                                                conditionValueKey = string.Format("{0}.choose.condition.{1}", paramKey, j);
                                                conditionValue = BaseCache.Get(conditionValueKey, RedisDb.Xml).ToStr().ToLower();
                                                conditionValue = conditionValue.Replace(temp.ParameterName.ToLower(), temp.Value.ToStr());
                                                if (BaseCodeDom.GetResult(conditionValue))
                                                {
                                                    isSuccess = true;
                        ...
                        if (tempSql.ToString() != "")
                        {
                            sql.Append(BaseCache.Get(dynKey, RedisDb.Xml));
        ...
        private static bool SaveXml(string key, FileInfo info, ConfigModel config, DataContext db)
        {
            if (config.IsMapSave)
            {
                ...
                {
                    var model = new Fast.DataModel.MySql.Data_MapFile();
                    model.MapId = key;
                    var query = FastRead.Query<Fast.DataModel.MySql.Data_MapFile>(a => a.MapId == key, null);
                    ...
                    if (query.ToCount(db) == 0)
                    {
                        ...
                        return db.Update<Fast.DataModel.MySql.Data_MapFile>(model, a => a.MapId == model.MapId, a => new { a.LastTime, a.EnFileContent, a.DeFileContent }).writeReturn.IsSuccess;
                }

                if (config.DbType == DataDbType.Oracle)
                {
                    var model = new Fast.DataModel.Oracle.Data_MapFile();
                    model.MapId = key;
                    var query = FastRead.Query<Fast.DataModel.Oracle.Data_MapFile>(a => a.MapId == key, null);
                    ...
                    if (query.ToCount(db) == 0)
                    {
                        ...
                        return db.Update<Fast.DataModel.Oracle.Data_MapFile>(model, a => a.MapId == model.MapId, a => new { a.LastTime, a.EnFileContent, a.DeFileContent }).writeReturn.IsSuccess;
                }

                if (config.DbType == DataDbType.SqlServer)
                {
                    var model = new Fast.DataModel.SqlServer.Data_MapFile();
                    model.MapId = key;
                    var query = FastRead.Query<Fast.DataModel.SqlServer.Data_MapFile>(a => a.MapId == key, null);
                    ...
                    if (query.ToCount(db) == 0)
                    {
                        ...
                        return db.Update<Fast.DataModel.SqlServer.Data_MapFile>(model, a => a.MapId == model.MapId, a => new { a.LastTime, a.EnFileContent, a.DeFileContent }).writeReturn.IsSuccess;
                }
            }
