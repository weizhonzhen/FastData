...
namespace Data
{
    ...
    public static class LambdaMap
    {
        ...
        public static void InstanceTable(Assembly[] list, string nameSpace, string dll, string dbKey = null)
        {
            ...
            {
                var listInfo = typeof(Data.DataModel.MySql.Data_MapFile).GetProperties(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly).ToList();
                var listAttribute = typeof(Data.DataModel.MySql.Data_MapFile).GetTypeInfo().GetCustomAttributes().ToList();
                BaseTable.Check(query, "Data_MapFile", "Data.DataModel.MySql", listInfo, listAttribute);
            }

            if (query.Config.DbType == DataDbType.SqlServer)
            {
                var listInfo = typeof(Data.DataModel.SqlServer.Data_MapFile).GetProperties(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly).ToList();
                var listAttribute = typeof(Data.DataModel.SqlServer.Data_MapFile).GetTypeInfo().GetCustomAttributes().ToList();
                BaseTable.Check(query, "Data_MapFile", "Data.DataModel.SqlServer", listInfo, listAttribute);
            }
        }
        #endregion
        ...
        public static void InstanceMap(string dbKey = null)
        {
            ...
            var config = DataConfig.GetConfig(true, dbKey);
        ...
        private static PageResult ExecuteSqlPage(PageModel pModel, string sql, DbParameter[] param, ReadContext db = null, string key = null)
        {
            ...
            var config = DataConfig.GetConfig(true, key);
